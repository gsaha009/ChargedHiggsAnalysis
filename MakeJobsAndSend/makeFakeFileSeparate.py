...
def getSRandFakeRootFiles(postHaddFilePath, isSignal=False):
    ...
    if (haddfile.IsZombie()):
        ...
        SR_histList = [key.GetName() for key in haddfile.GetListOfKeys() if not any(['SB' in key.GetName(),'ObjectSelection' in key.GetName()])]
        SB_file  = ROOT.TFile.Open(SB_fileName, 'RECREATE')
        if not isSignal:
            SB_histList = [key.GetName() for key in haddfile.GetListOfKeys() if not any(['SR' in key.GetName(),'ObjectSelection' in key.GetName()])]
        ...
        with alive_bar(len(SR_histList), bar='circles') as SR_bar:
            for histName in SR_histList:
                time.sleep(0.03)
                hist = copy.deepcopy(haddfile.Get(histName))
                hist.SetName(histName.replace('_SR',''))
                SR_file.cd()
                hist.Write()
                SR_bar()
        SR_file.Close()
        ...
        if not isSignal:
            with alive_bar(len(SR_histList), bar='circles') as SB_bar:
                for histName in SB_histList:
                    time.sleep(0.03)
                    hist = copy.deepcopy(haddfile.Get(histName))
                    hist.SetName(histName.replace('_SB',''))
                    SB_file.cd()
                    hist.Write()
                    SB_bar()
        SB_file.Close()
